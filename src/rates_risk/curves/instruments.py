...
class CurveInstrument(ABC):
    ...
    def __post_init__(self) -> None:
        """Fail fast on malformed market inputs.

        Curve construction is a trust boundary: non-finite quotes and empty
        tenors must never be allowed to turn into apparently successful curve
        nodes downstream.
        """
        if not isinstance(self.tenor, str) or not self.tenor.strip():
            raise ValueError("Instrument tenor must be a non-empty string")
        self.tenor = self.tenor.strip().upper()
        if not np.isfinite(self.quote):
            raise ValueError(f"Quote for {self.tenor} must be finite, got {self.quote!r}")
        self.quote = float(self.quote)
        quote_type = str(self.quote_type).strip().upper()
        if quote_type not in {"RATE", "PRICE"}:
            raise ValueError(f"Unsupported quote_type {self.quote_type!r} for {self.tenor}")
        self.quote_type = quote_type
        if not isinstance(self.day_count, DayCount):
            raise TypeError("day_count must be a DayCount value")
...
class FRA(CurveInstrument):
    ...
    def __post_init__(self):
        ...
        if not self.start_tenor:
            ...
            self.start_tenor = "0D"
        if not isinstance(self.start_tenor, str):
            raise ValueError("FRA start_tenor must be a string")
        self.start_tenor = self.start_tenor.strip().upper()
