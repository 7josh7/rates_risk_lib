...
def _validated_inputs(
    ...
    *,
    ...
    """Return finite, one-dimensional inputs sorted on a strict time grid."""
    times_array = np.asarray(times, dtype=np.float64)
    values_array = np.asarray(values, dtype=np.float64)
    if times_array.ndim != 1 or values_array.ndim != 1:
        raise ValueError("Times and interpolation values must be one-dimensional")
    ...
    if not np.all(np.isfinite(times_array)) or not np.all(np.isfinite(values_array)):
        raise ValueError(f"Times and {value_name} must be finite")
    ...
    if np.any(np.diff(times_array) <= 0):
        raise ValueError("Interpolation times must be strictly increasing")
...
class LinearInterpolator(Interpolator):
    ...
    def fit(self, times: np.ndarray, values: np.ndarray) -> None:
        ...
        self.times, self.values = _validated_inputs(times, values)
...
class LogLinearInterpolator(Interpolator):
    ...
    def fit(self, times: np.ndarray, discount_factors: np.ndarray) -> None:
        ...
        """
        times_array, discount_array = _validated_inputs(
            times,
            discount_factors,
            value_name="discount factors",
        )
        if np.any(discount_array <= 0):
            raise ValueError("Discount factors must be positive")
        self.times = times_array
        self.log_df = np.log(discount_array)
