...
class CurveNode:
    ...
    def from_discount_factor(cls, time: float, df: float) -> "CurveNode":
        ...
        if not np.isfinite(time) or time < 0:
            raise ValueError(f"Node time must be finite and non-negative: {time}")
        if not np.isfinite(df) or df <= 0:
            raise ValueError(f"Discount factor must be positive and finite: {df}")
        if time <= 0:
    ...
    def from_zero_rate(cls, time: float, zr: float) -> "CurveNode":
        ...
        if not np.isfinite(zr):
            raise ValueError(f"Zero rate must be finite: {zr}")
        df = np.exp(-zr * time)
        return cls(time=time, discount_factor=df, zero_rate=zr)
...
class Curve:
    ...
    def add_node(self, time: float, discount_factor: float) -> None:
        ...
        if not np.isfinite(discount_factor) or discount_factor <= 0:
            raise ValueError(f"Discount factor must be positive and finite: {discount_factor}")
    ...
    def build(self) -> None:
        ...
        times = np.array([n.time for n in self._nodes], dtype=float)
        zero_rates = np.array([n.zero_rate for n in self._nodes], dtype=float)
        discount_factors = np.array([n.discount_factor for n in self._nodes], dtype=float)

        if not np.all(np.isfinite(times)) or not np.all(np.isfinite(zero_rates)):
            raise ValueError("Curve nodes contain non-finite times or zero rates")
        if not np.all(np.isfinite(discount_factors)) or np.any(discount_factors <= 0):
            raise ValueError("Curve nodes contain invalid discount factors")
        if np.any(np.diff(times) <= 0):
            raise ValueError("Curve node times must be strictly increasing")
        
        self._interpolator = create_interpolator(self.interpolation_method)
        values = discount_factors if isinstance(self._interpolator, LogLinearInterpolator) else zero_rates
        self._interpolator.fit(times, values)
    ...
    def discount_factor(self, t: Union[float, date]) -> float:
        ...
        self._ensure_fitted()
        if isinstance(self._interpolator, LogLinearInterpolator):
            df = self._interpolator.get_discount_factor(float(t))
        else:
            zr = self._interpolator.interpolate(float(t))
            df = np.exp(-zr * float(t))
        if not np.isfinite(df) or df <= 0:
            raise ValueError(f"Interpolated discount factor is invalid at t={t}: {df}")
        return float(df)
    ...
    def zero_rate(
        ...
        self._ensure_fitted()
        if isinstance(self._interpolator, LogLinearInterpolator):
            zr_cont = -self._interpolator.interpolate(float(t)) / float(t)
        else:
            zr_cont = self._interpolator.interpolate(float(t))
        if not np.isfinite(zr_cont):
            raise ValueError(f"Interpolated zero rate is non-finite at t={t}")
    ...
    def instantaneous_forward(self, t: Union[float, date]) -> float:
        ...
        self._ensure_fitted()
        if isinstance(self._interpolator, LogLinearInterpolator):
            forward = -self._interpolator.derivative(float(t))
        else:
            zr = self._interpolator.interpolate(float(t))
            dz_dt = self._interpolator.derivative(float(t))
            forward = zr + float(t) * dz_dt
        if not np.isfinite(forward):
            raise ValueError(f"Instantaneous forward is non-finite at t={t}")
