...
class SabrParams:
    ...
    which is more stable for calibration workflows and aligns with common desk
    conventions. ``sigma_atm`` is the Black-equivalent ATM volatility used to
    invert alpha. Normal market quotes are converted explicitly by the
    calibrator and retained separately in ``CalibrationResult``.
    
    Attributes:
        sigma_atm: Black-equivalent ATM implied volatility used for alpha inversion
    ...
    def __post_init__(self):
        ...
        non_finite = [name for name, value in values.items() if not np.isfinite(value)]
        if non_finite:
            raise ValueError(f"SABR parameters must be finite: {non_finite}")
        if not -1 < self.rho < 1:
