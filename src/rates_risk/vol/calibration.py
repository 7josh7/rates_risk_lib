...
class CalibrationResult:
    ...
    iterations: int = 0
    market_atm_vol: float = float("nan")
    market_vol_type: str = ""


class SabrCalibrationError(RuntimeError):
    """Raised when SABR calibration cannot produce a finite optimizer result."""
...
class SabrCalibrator:
    ...
    def fit(
        ...
        """
        vol_type = str(vol_type).strip().upper()
        if vol_type not in {"NORMAL", "LOGNORMAL"}:
            raise ValueError(f"vol_type must be NORMAL or LOGNORMAL, got {vol_type!r}")
        if not np.isfinite(F) or not np.isfinite(T) or not np.isfinite(shift):
            raise ValueError("F, T, and shift must be finite")
        if F + shift <= 0:
        ...
        required_columns = {"strike", "vol"}
        missing = required_columns.difference(quotes_df.columns)
        if missing:
            raise ValueError(f"Missing SABR quote columns: {sorted(missing)}")
        if quotes_df.empty:
            raise ValueError("At least one SABR quote is required")
        
        # Extract data
        strikes = quotes_df['strike'].to_numpy(dtype=float)
        vols = quotes_df['vol'].to_numpy(dtype=float)
        if not np.all(np.isfinite(strikes)) or not np.all(np.isfinite(vols)):
            raise ValueError("SABR strikes and volatilities must be finite")
        if np.any(vols <= 0):
            raise ValueError("SABR volatilities must be strictly positive")
        if vol_type == "LOGNORMAL" and np.any(strikes + shift <= 0):
            raise ValueError("Shifted strikes must be positive for lognormal SABR")
        
        # Handle weights - check if column exists in DataFrame
        if 'weight' in quotes_df.columns:
            weights = quotes_df['weight'].to_numpy(dtype=float)
        else:
            weights = np.ones(len(strikes))
        if not np.all(np.isfinite(weights)) or np.any(weights < 0) or weights.sum() <= 0:
            raise ValueError("SABR weights must be finite, non-negative, and sum to a positive value")
        ...
        if not result.success or not np.isfinite(result.fun) or result.fun > 1e-8:
            try:
                ...
                if np.isfinite(result_de.fun) and (
                    not np.isfinite(result.fun) or result_de.fun < result.fun
                ):
                    result = result_de
                    optimizer_method = "differential_evolution"
            except (ValueError, FloatingPointError, RuntimeError) as exc:
                if not result.success:
                    raise SabrCalibrationError(f"Global SABR optimization failed: {exc}") from exc
        ...
        if (
            ...
            raise SabrCalibrationError(
                f"SABR optimizer failed: {getattr(result, 'message', 'unknown failure')}"
            )
        ...
        return CalibrationResult(
            ...
            market_atm_vol=float(sigma_atm),
            market_vol_type=vol_type,
        )
