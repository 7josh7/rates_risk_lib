...
This reference is extracted from the package public surface. Keep it synchronized with exported symbols and tests.


The package-root API is listed first, so `from rates_risk import ...` works for every symbol in those sections. Public methods and properties defined on exported classes are listed under each class. Additional public exports that are only available from submodules are listed afterward.
...
Symbols in this appendix are public and importable from submodules such as `rates_risk.curves` or `rates_risk.var`, but they are not re-exported from the `rates_risk` package root.
