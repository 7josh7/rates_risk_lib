...
\begin{itemize}
    \item Calendar-aware settlement, tenor advancement, and coupon schedule generation
    \item Product and currency-aware market convention resolution
    \item Typed trade normalization while preserving legacy dictionary inputs
    ...
    \item VaR backtesting with Kupiec, Christoffersen, and traffic-light diagnostics
...
\code{calendars} & Weekend-only, explicit-holiday, and US federal-style business calendars \\
\code{dates} & Date utilities, tenor parsing, schedule generation \\
\code{domain} & Typed trade models, trade normalization, and pricing policy controls \\
\code{market\_conventions} & Named market templates and product/currency convention resolution \\
...
\code{var} & Historical simulation, Monte Carlo VaR, stressed VaR, scenario engine, backtesting \\
...
\section{Business Calendars}

The library supports explicit calendar objects in addition to the original weekend-only behavior. Calendar-aware functions accept \code{None}, a \code{BusinessCalendar} instance, or an iterable of holiday dates.

\begin{itemize}
    \item \code{WeekendOnlyCalendar}: Saturdays and Sundays are non-business days.
    \item \code{ExplicitHolidayCalendar}: weekends plus a caller-provided holiday set.
    \item \code{UnitedStatesHolidayCalendar}: practical US federal-style holiday rules for USD rates examples.
\end{itemize}

\begin{lstlisting}
from datetime import date
from rateslib import UnitedStatesHolidayCalendar, advance_business_days
from rateslib.dates import DateUtils

calendar = UnitedStatesHolidayCalendar()

# July 4, 2024 is a US holiday, so the following business day is July 5.
settlement = advance_business_days(date(2024, 7, 3), 1, calendar)

# Spot dates and tenor advancement can use the same calendar.
spot = DateUtils.spot_date(date(2024, 1, 15), settlement_days=2, holidays=calendar)
maturity = DateUtils.add_tenor(
    spot,
    "5Y",
    holidays=calendar,
...
For product-aware defaults, use \code{market\_conventions}. The dispatcher resolves templates from an explicit trade template, trade currency, or curve currency before falling back to legacy pricer defaults.

\begin{lstlisting}
from rateslib import available_market_conventions, resolve_market_convention_for_trade

templates = available_market_conventions()
trade = {"instrument_type": "SWAP", "currency": "USD"}
resolved = resolve_market_convention_for_trade(trade, curve_currency="USD")

print(sorted(templates))
print(resolved.template_id)  # USD_VANILLA_SWAP
\end{lstlisting}
...
\begin{lstlisting}
from datetime import date
from rateslib import DayCount
from rateslib.dates import generate_bond_schedule
...
\begin{lstlisting}
from rateslib import MarketState, CurveState, PricingPolicy
...
class CurveState:
    ...
    def copy(self, discount_curve: Optional[Curve] = None,
             projection_curve: Optional[Curve] = None) -> "CurveState"
\end{lstlisting}

\subsection{PricingPolicy}

\code{PricingPolicy} controls model fallback behavior and audit metadata in dispatcher-based pricing:

\begin{itemize}
    \item \code{sabr\_bucket\_fallback}: \code{"allow"}, \code{"warn"}, or \code{"error"}.
    \item \code{allow\_zero\_option\_vol}: whether options may price with zero-vol fallback when neither explicit vol nor SABR parameters are available.
    \item \code{record\_audit}: whether \code{price\_trade} includes pricing-policy, market-convention, and SABR lookup details.
\end{itemize}
...
for failure in result.failed_trades:
...
\begin{enumerate}[leftmargin=1.2em,itemsep=0.2em]
    ...
    \item \checkmark Pricing policies and structured SABR lookup diagnostics for controlled fallback behavior.
...
ois_curve = bootstrap_from_quotes(valuation_date, curve_quotes)
...
print(f"Forward: {details['forward']*100:.3f}%")
print(f"Implied Vol: {details['implied_vol']*10000:.1f} bp")
...
\section{Package Root Exports}

The package root intentionally re-exports the primary workflow objects so examples can import from \code{rateslib} without reaching into implementation modules. The current root API includes the following groups:

\begin{lstlisting}
# Version
__version__

# Calendars, conventions, and dates
BusinessCalendar, WeekendOnlyCalendar, ExplicitHolidayCalendar,
UnitedStatesHolidayCalendar, DayCount, BusinessDayConvention,
Conventions, advance_business_days, year_fraction,
DateUtils, ScheduleInfo

# Typed trades, policies, and market conventions
PricingPolicy, TradeNormalizationError, TradeBase, BondTrade,
SwapTrade, FuturesTrade, SwaptionTrade, CapletTrade, normalize_trade,
ConventionSpec, MarketConventionTemplate, ResolvedMarketConvention,
MarketConventionError, STANDARD_MARKET_TEMPLATES,
available_market_conventions, get_market_convention,
resolve_market_convention_for_trade

# Curves, pricers, dispatcher, and portfolio builders
Curve, OISBootstrapper, NelsonSiegelSvensson,
LinearInterpolator, CubicSplineInterpolator, LogLinearInterpolator,
BondPricer, SwapPricer, FuturesPricer, FuturesContract,
price_trade, risk_trade, PricerOutput,
build_bond_trade, build_swap_trade, build_swaption_trade,
build_caplet_trade, build_typed_trade_from_position,
build_trade_from_position, price_portfolio_with_diagnostics,
PortfolioPricingResult, TradeFailure, SIGN_LONG, SIGN_SHORT,
PositionValidationError, MissingFieldError, InvalidOptionError

# Risk, VaR, reporting, liquidity, vol, and options
BumpEngine, RiskCalculator, KeyRateEngine, InstrumentRisk,
PortfolioRisk, LimitDefinition, LimitResult, DEFAULT_LIMITS,
evaluate_limits, limits_to_table, compute_limit_metrics,
HistoricalSimulation, MonteCarloVaR, StressedVaR, ScenarioEngine,
STANDARD_SCENARIOS, VaRBacktestResult, backtest_var,
kupiec_pof_test, christoffersen_independence_test, traffic_light,
rolling_historical_var_backtest, PnLAttributionEngine, PnLAttribution,
RiskReport, ReportFormatter, export_to_csv,
LiquidityEngine, LiquidityAdjustedVaR,
SabrParams, SabrModel, SabrCalibrator, VolQuote, load_vol_quotes,
hagan_black_vol, hagan_normal_vol, normalize_vol_quotes,
SabrSurfaceState, SabrBucketParams, SabrLookupResult,
make_bucket_key, calibrate_sabr_bucket, build_sabr_surface,
bachelier_call, bachelier_put, black76_call, black76_put,
shifted_black_call, shifted_black_put, bachelier_greeks,
black76_greeks, CapletPricer, SwaptionPricer, SabrOptionRisk,
...
The \code{domain} module is the boundary between external dictionary payloads and internal pricing/risk engines. Existing dictionary APIs remain supported, but dispatcher workflows normalize inputs into typed trade dataclasses.

\begin{lstlisting}
@dataclass(frozen=True)
class PricingPolicy:
    sabr_bucket_fallback: str = "allow"  # "allow", "warn", or "error"
...
\section{Market Conventions Module}
...
\subsection{Interpolation Classes}
...
\subsection{FuturesPricer Class}
...
class FuturesPricer:
    ...
    def implied_rate(self, expiry: date, underlying_tenor: str = "3M") -> float
    def price_from_rate(self, rate: float) -> float
    def rate_from_price(self, price: float) -> float
    def theoretical_price(self, contract: FuturesContract) -> float
    def dv01(self, contract: FuturesContract, num_contracts: int = 1) -> float
    def position_pv(
        self,
...
def build_typed_trade_from_position(
    ...
    """Dispatch and return a typed trade object."""
\end{lstlisting}
...
class CurveState:
    ...
    def copy(
        self,
        discount_curve: Optional[Curve] = None,
        projection_curve: Optional[Curve] = None
    ) -> "CurveState"
\end{lstlisting}
...
class SabrSurfaceState:
    ...
    Attributes:
        ...
        missing_bucket_policy: fallback behavior; "nearest" is the supported search policy
    """
    ...
    def resolve_bucket(
        ...
        """Return exact/fallback SABR lookup details without mutating diagnostics."""
    
    def diagnostics_table(self) -> Dict[BucketKey, Dict[str, Any]]:
        """Return diagnostics keyed by bucket for reporting."""
\end{lstlisting}

\subsection{SabrLookupResult Dataclass}
...
\subsection{Base Model Functions}

\begin{lstlisting}
def bachelier_call(F: float, K: float, T: float, sigma: float, annuity: float = 1.0) -> float
def bachelier_put(F: float, K: float, T: float, sigma: float, annuity: float = 1.0) -> float
def black76_call(F: float, K: float, T: float, sigma: float, annuity: float = 1.0) -> float
def black76_put(F: float, K: float, T: float, sigma: float, annuity: float = 1.0) -> float
...
def bachelier_greeks(...) -> Dict[str, float]
def black76_greeks(...) -> Dict[str, float]
\end{lstlisting}
...
class CapletPricer:
    ...
    def price_with_sabr(
        ...
        """Price caplet/floorlet using SABR implied vol."""
\end{lstlisting}
...
%===============================================================================
% P&L, LIQUIDITY, AND RISK REPORTING
%===============================================================================
...
\subsection{Report Formatting and CSV Export}
